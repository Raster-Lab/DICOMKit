...
- **`dicom-convert` output-dir handling**: When `--output` points to an existing directory or an extension-less path, the tool auto-creates the directory and names the output file after the input stem — matching CLI Workshop behaviour. Release binary rebuilt to match.
- **Transfer syntax labels for JPEG 2000 Part 2 and HTJ2K** (`ImageMetadataHelpers.transferSyntaxLabel`): UIDs `.92`, `.93`, `.201`, `.202`, `.203` now resolve to human-readable names instead of falling through to the raw UID string. Previously the DICOM Inspector header showed the raw UID for any HTJ2K or Part 2 file.
- **JPEG 2000 Part 2 and HTJ2K variants in CLI Workshop**: `dicom-convert` transfer syntax dropdown and `CLIWorkshopHelpers` now include `JPEG2000Part2Lossless`, `JPEG2000Part2`, `HTJ2KLossless`, `HTJ2KRPCLLossless`, `HTJ2K`. Parser aliases added to `CLIWorkshopViewModel.parseTransferSyntax`. Unknown-TS error message now lists all supported syntaxes.
- **DICOM Inspector Transfer Syntax header row**: First row of the inspector list now shows `Transfer Syntax: <name> (<UID>)` sourced from File Meta Information tag `(0002,0010)` so users can immediately verify which encoding is loaded without scrolling.
- **Inspector live-view binding**: Inspector sheet now binds to the `ImageViewerViewModel` directly (`DICOMInspectorView(viewModel:)`) instead of capturing a static `DICOMFile` snapshot, so it updates automatically when the viewer loads a new file.
- **Library auto-select after import**: After importing or re-importing a file, `StudyBrowserViewModel` sets `selectedStudyUID` to the newly imported study and `StudyBrowserView` scrolls to it via `ScrollViewReader`.

### Fixed

- **Encapsulated pixel data serialization** (`DICOMWriter.serializeElement`): Fragment items (JPEG 2000, HTJ2K, JPEG) were silently dropped when writing an element with `length == 0xFFFFFFFF` and non-nil `encapsulatedFragments`. The writer now emits the full sequence: Basic Offset Table item → Fragment items (even-padded) → Sequence Delimitation Item. This was the root cause of JPEG2000Lossless / HTJ2KLossless → ImplicitVRLittleEndian decompressed files being 14 KB (headers only) instead of the expected 12–15 MB.
- **HTJ2K lossy bitrate collapse at high bit depths** (`J2KSwiftCodec.makeEncodingConfiguration`): The upstream quality→BPP map (quality 0.90 → 1.2 bpp) caused visually-blank reconstructions for `BitsStored ≥ 9` (e.g. 12-bit X-ray). Bitrate is now anchored to the source bit depth: quality 0.90 → 50 % of `bitsStored` → ≥ 4 bpp for a typical 12-bit image. `PixelDataDescriptor` is now forwarded to the encoding configuration and round-trip verifier.
- **J2K decode via J2KSwiftCodec, not ImageIO** (`DICOMFile+PixelData.imageIODecodedTransferSyntaxes`): JPEG 2000 UIDs (`.90`, `.91`) removed from the ImageIO fast-path set. ImageIO's implicit YCbCr→RGB conversion during decode corrupted photometric interpretation; `J2KSwiftCodec` handles these syntaxes and lets the renderer perform the colour transform explicitly.
- **Stale file served from Library after re-import** (`ImportService.importFile`): When a file with an existing SOP Instance UID was re-imported (e.g. a transcoded IVRLE variant of a previously-imported JPEG 2000 Lossless instance), the copy-to-managed-storage step was skipped due to a `!fm.fileExists` guard. The Library therefore handed the old bytes to the Viewer, and the Inspector showed the previous transfer syntax. Fix: compare existing cached bytes to the newly-imported bytes; overwrite when they differ, skip only when identical.
- **Scroll-wheel zoom during Inspector sheet** (`ImageViewerView.ScrollWheelHandler`): Scroll events over the DICOM Inspector (and any other attached sheet) were intercepted by the image zoom handler. The local `NSEvent` monitor now rejects events when `window.attachedSheet != nil`, when the target window doesn't match, or when the hit-tested view is outside the image subtree.
- **Inspector showing wrong transfer syntax for JPEG 2000 / HTJ2K files**: Combination of the encapsulated-serialization bug (which caused decompressed outputs to be nearly empty) and the stale-library-cache bug (which served the old file back to the viewer). Both bugs fixed above; Inspector header reliably reflects the file actually in the viewer.
