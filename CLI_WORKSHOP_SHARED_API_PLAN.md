...
|------|------|--------------|----------|
| 2026-06-08 | **Wave 4 (Bucket B)** `dicom-export` | Extracted the pure/deterministic image-export logic into a new `Sources/DICOMKit/ImageExport/DICOMImageExporter.swift` (public `DICOMImageExporter` namespace + `ExportImageFormat`/`OrganizationScheme`/`ExportError`): EXIF mapping (`mapDICOMFieldToEXIF`/`getDICOMFieldValue`/`buildEXIFMetadata`), contact-sheet geometry (`contactSheetLayout`/`thumbnailPosition`), animation (`gifFrameDelay`/`validatedFrameRange`), bulk path organization (`sanitizePathComponent`/`buildOrganizedPath`), `determineWindowSettings`, and `exportCGImage`. Builds on the already-shared `DICOMFile.tryRenderFrame`. CLI main.swift: removed those defs (660→~545 lines), qualified call sites, kept the CLI-only `ExpressibleByArgument` conformances. App `executeDicomExport`: replaced its inline duplicates with thin wrappers delegating to the shared engine (deleted `mapDICOMFieldToEXIF`/`getDICOMFieldValue`/`utTypeIdentifier`/`ExportRenderError` + the EXIF/window bodies); kept app-specific `renderFrame`/`collectDICOMFiles` + the 4-mode orchestration (sandbox `OutputAccess`). | `swift build` green; CLI smoke: single PNG, single JPEG **with EXIF** (description/manufacturer/software tags present), contact-sheet (2×1 grid). No goldens (non-deterministic image bytes) → verified by smoke + transitive engine sharing. Parity unchanged: MATCH=266, DIFFERS=0, strict gate green. |
| 2026-06-08 | **Wave 4 (Bucket B)** `dicom-compress` | Moved the 825-line `CompressionManager` (+ `CompressionInfo`, `TransferSyntaxHelper`, `CompressionError`) out of `Sources/dicom-compress/` into `Sources/DICOMKit/Compression/`. Made only the entry points public (`transferSyntax`, `transferSyntaxDisplayName`, `supportedCodecs`, `findDICOMFiles`, `getCompressionInfo`, `compressFile`, `decompressFile`, `CompressionInfo`, `CompressionError`); the codec-dispatch + Part-10 serialization helpers stay internal. Added in-memory `compressData(_:codec:quality:)`, `decompressData(_:syntax:)`, and `getCompressionInfo(data:)` so DICOMStudio writes via `OutputAccess`; the file methods now wrap them. App deleted ~250 lines of duplicated `dcCompress*` transform helpers (encode/decode/transcode/descriptor/offset-table/serialize/write) and repointed info/compress/decompress/batch to the shared engine (kept only the `dcCompressFormatBytes`/`dcCompressParseQuality` display utils). | `swift build` green; CLI smoke: info, compress→RLE, decompress round-trip (Explicit→RLE→Explicit). Regenerated goldens **unchanged**; all 4 `dicom-compress` parity scenarios stay MATCH — MATCH=266, DIFFERS=0, strict gate green. |
| 2026-06-08 | **Wave 4 (Bucket B)** `dicom-uid` | Moved the `UIDManager` workflow (generate/validate/lookup/regenerate + `UIDManagerError`/`UIDValidationResult`/`UIDMapping`) out of `Sources/dicom-uid/UIDManager.swift` into `Sources/DICOMKit/UIDManagement/`, all public. Added a no-write `regenerateData(_:root:maintainRelationships:existingMappings:) -> (Data, [UIDMapping])` core (the `regenerateUIDs(inputPath:…)` disk version now wraps it) so DICOMStudio writes through its sandbox-aware `OutputAccess`. App `executeDicomUID*` deleted ~80 lines of duplicated logic (`validateUID` PS3.5 rules, `uidTypeDescription`, `tagName`, the generate + regenerate loops) and now calls the shared engine. Qualified `DICOMKit.UIDValidationResult` (DICOMStudio has its own same-named type). | `swift build` green; CLI smoke (validate valid/invalid, lookup, regenerate dry-run + real → valid DICOM). Regenerated goldens **unchanged**; all 5 `dicom-uid` parity scenarios stay MATCH — MATCH=266, DIFFERS=0, strict gate green. (Also regenerated `CLIContracts.json` to drop stale trailing whitespace in the Wave-3 `dicom-image` help text.) |
...
|------|-----------------------|------------------------------|
| `dicom-uid` ✅ **DONE** | `UIDGenerator` (DICOMCore), `UIDDictionary` (DICOMDictionary) | ~~`Sources/dicom-uid/UIDManager.swift`~~ → now `Sources/DICOMKit/UIDManagement/` | `UIDManager` shared (incl. no-write `regenerateData`); app calls it; parity MATCHES |
| `dicom-compress` ✅ **DONE** | `CodecRegistry` (DICOMCore) | ~~`Sources/dicom-compress/CompressionManager.swift`~~ → now `Sources/DICOMKit/Compression/` | `CompressionManager` shared (incl. in-memory `compressData`/`decompressData`/`getCompressionInfo(data:)`); app calls it; parity MATCHES |
| `dicom-export` ✅ **DONE** | render primitives (`DICOMFile.tryRenderFrame`, DICOMKit) | ~~inlined in both~~ → now `Sources/DICOMKit/ImageExport/DICOMImageExporter.swift` | `DICOMImageExporter` shares the deterministic export helpers (EXIF/layout/paths/window/encode); both adapters call it (no goldens — smoke-verified) |
...
| `dicom-uid` ✅ | B→done | `executeDicomUID*` → shared `UIDManager` | **done:** `UIDManager` in DICOMKit | `Sources/DICOMKit/UIDManagement/` (shared) |
| `dicom-compress` ✅ | B→done | `executeDicomCompress*` → shared `CompressionManager` | **done:** `CompressionManager` in DICOMKit | `Sources/DICOMKit/Compression/` (shared) |
| `dicom-export` ✅ | B→done | `executeDicomExport` → shared `DICOMImageExporter` | **done:** export helpers in DICOMKit | `Sources/DICOMKit/ImageExport/` (shared) |
