...
| B2 — Stale or incorrect, no citation | 10 | Data gap or bug relative to 2026a, undocumented | 🔄 In progress — 7 of 10 done (P1, P3, P5, P6 SRDocumentType) |
...
| [StructuredReporting/ContextGroup.swift](Sources/DICOMCore/StructuredReporting/ContextGroup.swift#L115) | **Text-diffed 2026-09-25** against PS3.16 2026a, all 9 CID tables by `xml:id`. **Only CID 244 (Laterality) is right:** its 4 codes are the `Include CID 247` row plus Bilateral and Unilateral. **CID 7021** has 2 wrong meanings (126002 is "Dynamic Contrast MR Measurement Report", 126003 is "PET Measurement Report"). **The other 7 CID numbers name different context groups:** 218 is "Quantitative Image Feature" (not Temporal Relation), 4021 is "PET Radiopharmaceutical" (not Finding Site), 6147 is "Response Criteria" (WHO/RECIST/RANO, not response codes), 7464 is "General ROI Measurement Modifier" (not units), 12301 is "Measurement Selection Reason" (not Imaging Observations), 6051 is "Breast Procedure Reason" (Breast Imaging Finding is CID 6054), 6024 is "Depth" (not Derivation). **Their members are largely invented:** the SRT codes R-40899 to R-4089D and F-01790/1/6 exist in no CID; DCM 121110-121112 mean Patient Presentation / Summary / Source of Measurement, not Baseline; DCM 126000-126005 as response codes are wrong (126004/5 do not exist; real lesion-response codes are CID 6144, 112041-112047); in "Derivation", 258090004 is labelled Estimated but means Calculated, and 258224002 does not exist (Measured is 258104002, CID 3627). **P6 confirmed and understated:** both the SRT-vs-SCT point and the 126000 reuse are real, but the root cause is that the groups were written from memory. **Awaiting a decision** on the public API (see Progress log). |
