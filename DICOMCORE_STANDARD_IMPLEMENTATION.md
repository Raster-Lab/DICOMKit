...
| B2 — Stale or incorrect, no citation | 10 | Data gap or bug relative to 2026a, undocumented | 🔄 In progress — 6 of 10 done (P1, P3, all of P5) |
...
| [StructuredReporting/SRDocumentType.swift](Sources/DICOMCore/StructuredReporting/SRDocumentType.swift#L12) | **Text-diffed 2026-09-25** against PS3.6 2026a Table A-1 (the 24 `…88.*` SOP Classes) and PS3.3 2026a A.35.1–A.35.23 (each IOD's Value Type constraints). **UIDs and names:** all 18 in the code match. **P6 confirmed:** Procedure Log Storage (.88.40) is missing. **Also missing:** Waveform Annotation SR Storage (.88.77), and the 4 retired "Trial" SR SOP Classes (.88.1–.88.4). **New finding, larger than P6:** `allowedValueTypes` is wrong for 16 of the 18 document types. Examples: Basic Text SR omits WAVEFORM; Enhanced SR omits SCOORD and TCOORD; Key Object Selection allows DATETIME (not permitted) and omits PNAME and WAVEFORM; the four Dose SRs allow DATE and TIME (not permitted); Implantation Plan is given every type but the IOD allows 9; Extensible SR allows every value type incl. TABLE, which `ContentItemValueType` lacks. Only Comprehensive SR and Comprehensive 3D SR are right. The sets were evidently written from memory. ⏸ **Awaiting approval:** 2 new public enum cases, a `.table` case on `ContentItemValueType`, and the data correction, which tightens validation. |
