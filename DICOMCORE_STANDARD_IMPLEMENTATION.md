...
| B2 — Stale or incorrect, no citation | 10 | Data gap or bug relative to 2026a, undocumented | 🔄 In progress — 4 of 10 done (P1, P3, P5 character sets) |
...
| [DirectoryRecord.swift:9-126](Sources/DICOMCore/DirectoryRecord.swift#L9) | **Text-diffed 2026-09-24** against PS3.3 2026a Table F.3-3, the (0004,1430) Enumerated Values (35 current). **P5 is confirmed and was incomplete.** 27 of 35 are present. 8 are missing: PLAN, TRACT, ASSESSMENT, RADIOTHERAPY, ANNOTATION, INVENTORY, **WF PRESENTATION** and **PRIVATE**. The 11 retired values in the code (OVERLAY … STORED PRINT, retired in PS3.3-2004; HL7 STRUC DOC, retired in 2018b) are genuinely retired but not labelled as such. PRINT QUEUE, FILM SESSION, FILM BOX, IMAGE BOX and MRDR are absent. `ROOT` is not a DICOM value; it is an internal placeholder. **Related in-module bug:** `DICOMDirectory.validate` (DICOMDirectory.swift:277-305) allows 7 of the 25 SERIES child types in Table F.4-1 and rejects PRIVATE children, so valid DICOMDIRs fail validation. ⏸ **Awaiting approval:** new public enum cases. |
