...
| B2 — Stale or incorrect, no citation | 10 | Data gap or bug relative to 2026a, undocumented | 🔄 In progress — 5 of 10 done (P1, P3, P5 character sets, P5 record types) |
...
| 2026-09-24 | B2 / P5 | `DICOMDirectory.swift` (`DICOMDIRProfile`) text-diffed against PS3.11 2026a: 5 of 8 identifiers are not real profile IDs, and about 56 are missing. Paused: the fix needs a public API design choice, because the Ultrasound identifiers are templated by media, which an enum cannot express. |
...
| [DICOMDirectory.swift:9-33](Sources/DICOMCore/DICOMDirectory.swift#L9) `DICOMDIRProfile` | **Text-diffed 2026-09-24** against PS3.11 2026a Annexes A–N: 64 profile identifiers, 6 of them Ultrasound templates `STD-US-{ID,SC,CC}-{SF,MF}-xxxx`, where xxxx is the media. Annex F (Waveform Diskette) is retired and has no identifier. **P5 is confirmed and understated:** only 3 of the enum's 8 identifiers exist (STD-GEN-CD, STD-GEN-DVD-J2K, STD-GEN-MIME). STD-GEN-DVD, STD-GEN-USB and STD-GEN-SEC are incomplete names; the real ones are, for example, STD-GEN-DVD-JPEG, STD-GEN-USB-JPEG/-J2K and STD-GEN-SEC-CD. STD-CTMR-xxxx and STD-US-xxxx are templates, not identifiers. **STD-MAM-xxxx does not exist in 2026a.** Missing entirely: all BD profiles (Annexes D, M, N), XA, Dental, DVD-MPEG2, ZIP-over-email, CF/SD/MMC flash, and every SEC variant. Impact is metadata only: no attribute carries the profile in a DICOMDIR, and `DICOMDIRReader` always sets `.standardGeneralCD`. It affects the `dicom-dcmdir --profile` option, DICOMStudio and the workflow summaries. ⏸ **Awaiting a design decision** on the public API (see Progress log). |
