...
| **dicom-merge** | `--format {enhanced-ct,enhanced-mr,enhanced-xa}` is **inert** — `FrameMerger.createMultiFrameFile` never reads `format`; all four values produce byte-identical *standard* multi-frame output. The Enhanced multi-frame construction (functional groups, per-frame sequences) is **not implemented**. *(Found during this pass; `--level study` IS real and is now tested.)* | ✅ **IMPLEMENTED** (Enhanced SOP class + Shared/Per-frame functional groups; 2 oracles added) |
...
| dicom-merge | ✅ `--level study`; ✅ `--sort-by ImagePositionPatient`; ✅ `--sort-by AcquisitionTime`; ⚙️ `--format enhanced-*` (inert — code issue) |
