...
private let verifiedVerdict: [String: EngineVerdict] = [
    ...
    "dicom-script":   .init(engine: "ScriptParser / Executor / Validator / TemplateGenerator", module: "DICOMKit/Scripting", scope: "full", verdict: "ALL subcommands shared — `template`, `run` (dry-run/variables/parallel/verbose/log), and `validate` byte-identical. The app now drives the same ScriptExecutor/ScriptValidator as the CLI (previously it reimplemented a parsed-plan view) and the CLI routes the engine log to stdout."),
