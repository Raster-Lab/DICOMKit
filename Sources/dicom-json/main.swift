...
struct DICOMJson: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            Converts DICOM files to JSON format (DICOM JSON Model) and vice versa.
            Supports DICOMweb JSON format with bulk data handling.
            
            Examples:
              dicom-json file.dcm --output file.json
              dicom-json file.json --output file.dcm --reverse
              dicom-json file.dcm --format dicomweb --pretty
    ...
    @Flag(name: .long, help: "Use streaming for large files")
    ...
    mutating func run() throws {
        ...
        if let specifiedOutput = output {
            ...
            if reverse {
                ...
                outputPath = inputURL.deletingPathExtension().appendingPathExtension("json").path
            }
        }
        ...
        if verbose {
            ...
            print("Mode:   \(reverse ? "JSON → DICOM" : "DICOM → JSON")")
            print()
        }
        
        // Perform conversion
        if reverse {
            try convertJsonToDicom(inputPath: input, outputPath: outputPath)
        } else {
            try convertDicomToJson(inputPath: input, outputPath: outputPath)
        }
    ...
    private func convertDicomToJson(inputPath: String, outputPath: String) throws {
        // Read DICOM file
        let startTime = Date()
        ...
        if metadataOnly {
            // Use metadata-only parsing option if available
            dicomFile = try DICOMFile.read(from: inputData)
        } else {
            dicomFile = try DICOMFile.read(from: inputData)
        }
        ...
        if verbose {
            ...
            print("Encoded to JSON: \(formatFileSize(Int64(jsonData.count))) in \(String(format: "%.2f", encodeTime))s")
        }
    ...
    private func convertJsonToDicom(inputPath: String, outputPath: String) throws {
        // Read JSON file
        let startTime = Date()
        let inputData = try Data(contentsOf: URL(fileURLWithPath: inputPath))
        ...
        if verbose {
            let readTime = Date().timeIntervalSince(startTime)
            print("Read JSON file: \(formatFileSize(Int64(inputData.count))) in \(String(format: "%.2f", readTime))s")
        }
        ...
        if verbose {
            ...
            print("Decoded JSON: \(elements.count) elements in \(String(format: "%.2f", decodeTime))s")
        }
...
enum JsonFormat: String, ExpressibleByArgument {
    case standard
    case dicomweb
    
    var defaultValueDescription: String {
        switch self {
        case .standard: return "Standard DICOM JSON Model"
        case .dicomweb: return "DICOMweb JSON format"
        }
    }
}
