...
**112 / 383 CLI flags (29.2%)** are exercised by ≥1 output scenario across 32 wired tools. The rest are the silent-coverage gap (the `⊘ not covered` flags above) — contract-driven auto-generation (plan Phase 2) drives this toward 100%. Machine-readable per-tool detail (incl. each tool's `uncoveredFlags`) is in `coverage.json`.
