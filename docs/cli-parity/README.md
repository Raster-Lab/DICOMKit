...
**133 / 383 CLI flags (34.7%)** are exercised by ≥1 output scenario across 32 wired tools. The 250 uncovered flags break down as: **170 network** (need a live PACS/DICOMweb server), **32 non-deterministic** (fresh UIDs/timestamps — no stable golden), **3 no-write preview**, and **45 coverage gap** (offline-testable, not yet templated). Only the **45 gap** flags are reducible by contract-driven auto-generation; the 205 network / non-deterministic / preview flags are a permanent floor whose App↔CLI parity is asserted by the per-tool **Verified** verdict (same shared DICOMKit engine → identical output by construction), not by a golden. Machine-readable per-tool detail (incl. each tool's `uncoveredFlags`) is in `coverage.json`.
