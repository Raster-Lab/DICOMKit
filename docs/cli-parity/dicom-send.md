# dicom-send

_CLI binary:_ `dicom-send` · _category:_ NETWORK_OPERATIONS · _wired in Studio:_ yes · _network:_ yes

**Input-contract parity:** 10/11 CLI flags matched · 1 missing in UI · status **INCOMPLETE** (91%)

**Output behavior:** no golden scenarios yet (offline output not exercised; e.g. network tool or not-yet-templated).

## Verified App↔CLI parity

> Manually audited 2026-06-09 at the code level, covering **every** flag — including
> those the auto-generated tables below mark `⊘ not covered`. Companion:
> [`APP_CLI_PARITY_MATRIX.md`](../../APP_CLI_PARITY_MATRIX.md) · [`APP_CLI_SHARED_API.md`](../../APP_CLI_SHARED_API.md).

- **Shared engine:** `DICOMStorageService` (`DICOMNetwork`) — both the CLI (`Sources/dicom-send/`) and DICOMStudio's `executeDicomSend*` call it (all logic shared). Flags with no golden therefore still produce **identical output by construction**.
- **Verified output match:** C-STORE execution runs through the identical shared service (same network I/O).
- **Intentional divergences (not bugs):** Console: app `✅`/`❌`/`⚠️` + **ms** + educational error hints; CLI `✓`/`✗` + **s**. Live network → no goldens.

## Flags

| Flag | Kind | Input (UI ↔ CLI) | Type/Default | Output (UI vs CLI) |
|---|---|---|---|---|
| `--aet` | option | ✅ match | ✓ | ⊘ not covered |
| `--called-aet` | option | ✅ match | ✓ | ⊘ not covered |
| `--dry-run` | flag | ✅ match | ✓ | ⊘ not covered |
| `--port` | option | ⚠️ missing in UI | — | ⊘ not covered |
| `--priority` | option | ✅ match | ✓ | ⊘ not covered |
| `--recursive` | flag | ✅ match | ✓ | ⊘ not covered |
| `--retry` | option | ✅ match | ✓ | ⊘ not covered |
| `--timeout` | option | ✅ match | ✓ | ⊘ not covered |
| `--transfer-syntax` | option | ✅ match | ✓ | ⊘ not covered |
| `--verbose` | flag | ✅ match | ✓ | ⊘ not covered |
| `--verify` | flag | ✅ match | ✓ | ⊘ not covered |

---
_Legend — Input:_ ✅ match · ⚠️ missing in UI · ➕ extra in UI (drift). _Output:_ ✅ success · ❌ drift · ⊘ not covered · — not wired. Generated by `swift run cli-parity-docs` (in-process, from bundled contracts + goldens)._
