...
- **Shared DICOMKit engine:** `DICOMwebClient (STOW-RS)` (`DICOMWeb`) — both the CLI and DICOMStudio call it (all logic shared); flags with no golden still produce identical output **by construction**.
- **Verdict:** STOW-RS store via the identical shared client. Live network → no goldens.
...
|---|---|---|---|---|
| `--batch` | option | ✅ match | ✓ | ⊘ not covered (network — needs a live PACS/DICOMweb server) |
| `--continue-on-error` | flag | ✅ match | ✓ | ⊘ not covered (network — needs a live PACS/DICOMweb server) |
| `--input` | option | ✅ match | ✓ | ⊘ not covered (network — needs a live PACS/DICOMweb server) |
